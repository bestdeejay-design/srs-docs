...
| № | Этап | ЧД | Зависимости |
|---|------|:---:|:---:|
| 1 | Регистрация организации на MAX, создание бота, модерация | 2 | Документы организации |
| 2 | Создание Telegram-бота, webhook, деплой | 0.5 | — |
| 3 | Redis-сессия `auth_session` (генерация, TTL, инвалидация) | 0.5 | — |
| 4 | Postgres `user_messenger_links` (миграция, CRUD) | 0.5 | — |
| 5 | Postgres `otp_codes` (миграция, CRUD, TTL) | 0.5 | — |
| 6 | Эндпоинт `POST /auth/request-otp` + deep link generation | 1 | №3, №4 |
| 7 | Эндпоинт `POST /auth/link` (внутренний, с секретом) | 1 | №4, №5 |
| 8 | Эндпоинт `POST /auth/verify-otp` | 0.5 | №5 |
| 9 | MAX Bot: webhook-обработчик `bot_started` | 1.5 | №7 |
| 10 | MAX Bot: отправка OTP `POST /messages` | 0.5 | №9 |
| 11 | Telegram Bot: webhook-обработчик `/start AUTH_XXX` + отправка | 1 | №7 |
| 12 | Frontend: экран входа (ввод телефона, кнопки MAX / TG, поле кода) | 1.5 | №6 |
| 13 | Frontend: экран привязки после успешного входа (опционально) | 1 | — |
| 14 | Rate limits, retry, таймауты, логирование | 1 | — |
| | **Итого** | **13.5** | |
