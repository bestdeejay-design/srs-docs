...
def build_html(input_md):
    ...
    for tag, attrs, text in headings:
        level, level_prev = int(tag[1]), int(tag[1]) if tag else 1
...
'''
    themes_css = ''.join(f'''
.theme-{name} {{ --bg: var(--{name}-bg); --bg2: var(--{name}-bg2); --text: var(--{name}-text);
  --text2: var(--{name}-text2); --border: var(--{name}-border); --accent: var(--{name}-accent);
  --accent2: var(--{name}-accent2) if name != 'dark' else 'var(--dark-accent2)'; --code-bg: var(--{name}-code); }}'''
        for name in ['apple', 'google', 'dark'])
...
body {{ font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,sans-serif;
  font-size:15px; line-height:1.6; color:var(--text); background:var(--bg); display:flex; }}
#sidebar {{ position:fixed; top:0; left:0; bottom:0; width:280px; background:var(--bg2);
...
#sidebar-header span {{ font-size:13px; font-weight:600; color:var(--accent); }}
.theme-btn {{ width:28px; height:28px; border-radius:50%; border:2px solid var(--border);
  cursor:pointer; padding:0; font-size:0; transition:transform .2s; }}
...
#toc-container a {{ display:block; padding:4px 20px 4px 10px; font-size:12.5px;
  color:var(--text2); text-decoration:none; border-radius:4px;
  transition:color .15s,background .15s; }}
#toc-container a:hover {{ color:var(--accent); background:rgba(0,0,0,0.04); }}
#search-wrap {{ padding:8px 12px; border-bottom:1px solid var(--border); position:relative; }}
#search {{ width:100%; padding:7px 32px 7px 10px; font-size:13px;
  border:1px solid var(--border); border-radius:8px; background:var(--bg);
  color:var(--text); outline:none; transition:border-color .2s; }}
#search:focus {{ border-color:var(--accent); }}
#search-clear {{ position:absolute; right:14px; top:50%; transform:translateY(-50%);
  width:20px; height:20px; border:none; border-radius:50%;
  background:var(--text2); color:var(--bg); font-size:12px; line-height:20px;
  text-align:center; cursor:pointer; display:none; padding:0; opacity:.6; }}
#search-clear:hover {{ opacity:1; }}
#search-results {{ display:none; position:absolute; top:100%; left:0; right:0;
...
#search-results a {{ display:block; padding:8px 14px; font-size:13px;
...
#content h2 {{ font-size:21px; font-weight:600; margin:36px 0 10px; padding-bottom:8px;
...
#content blockquote {{ border-left:4px solid var(--accent); padding:10px 18px; margin:14px 0;
  background:var(--bg2); color:var(--text2); border-radius:0 8px 8px 0; font-size:14px; }}
#content pre {{ background:var(--code-bg); padding:14px 18px; border-radius:8px;
  overflow-x:auto; font-size:13px; margin:14px 0;
...
#menu-toggle {{ display:none; position:fixed; top:10px; left:10px; z-index:200;
  ...
  background:var(--accent); color:#fff; font-size:18px; cursor:pointer; opacity:.9; }}
@media (max-width:768px) {{
  #sidebar {{ transform:translateX(-100%); }}
  #sidebar.open {{ transform:translateX(0); }}
  #content {{ margin-left:0; padding:56px 16px 40px; }}
...
document.querySelectorAll('#content h2, #content h3, #content h4').forEach(h => {{
  const id = h.id || '', text = h.textContent.trim();
  let el = h.nextElementSibling, ctx = '';
  while (el && !/^h[234]$/i.test(el.tagName)) {{ ctx += ' ' + (el.textContent||'').trim(); el = el.nextElementSibling; if (ctx.length > 200) break; }}
  searchIndex.push({{ id, text, context: ctx.trim().slice(0,300) }});
}});
function searchDocs(q) {{
  const r = document.getElementById('search-results');
  if (!q || q.length < 2) {{ r.classList.remove('show'); return; }}
  q = q.toLowerCase();
  const m = searchIndex.filter(e => e.text.toLowerCase().includes(q) || e.context.toLowerCase().includes(q)).slice(0,20);
  if (!m.length) {{ r.classList.remove('show'); return; }}
  r.innerHTML = m.map(x => '<a href="#'+x.id+'" onclick="toggleSidebar()"><div class="sr-title">'+esc(x.text)+'</div><div class="sr-context">'+esc(x.context.slice(0,120))+'</div></a>').join('');
...
function setTheme(name) {{
  ...
  document.querySelectorAll('.theme-btn').forEach(b => b.classList.toggle('active', b.classList.contains(name)));
  try {{ localStorage.setItem('srs-theme', name); }} catch(e) {{}}
}}
// Restore saved theme
try {{
  const t = localStorage.getItem('srs-theme');
  if (t && ['apple','google','dark'].includes(t)) setTheme(t);
}} catch(e) {{}}
</script></body></html>'''
