...
#sidebar-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.3);z-index:99;-webkit-tap-highlight-color:transparent}
#sidebar{position:fixed;top:0;left:0;bottom:0;width:280px;background:var(--bg2);border-right:1px solid var(--border);display:flex;flex-direction:column;z-index:100;transition:transform .25s}
...
#toc-container{flex:1;overflow-y:auto;padding:4px 0;-webkit-overflow-scrolling:touch}
#toc-container ul{list-style:none;padding:0;margin:0}
#toc-container>ul{padding-left:6px}
#toc-container li{padding:1px 0;position:relative}
#toc-container a{display:inline;padding:3px 6px 3px 2px;font-size:13px;color:var(--text2);text-decoration:none;line-height:1.4;border-radius:3px;transition:color .1s}
#toc-container .sw a{padding-left:6px}
...
#content{margin-left:280px;flex:1;max-width:960px;padding:40px 56px 80px}
...
#content p{margin-bottom:16px;word-wrap:break-word}
...
#content pre{background:var(--code);padding:14px 18px;border-radius:var(--radius);overflow-x:auto;font-size:13px;margin:16px 0;font-family:var(--font-mono);line-height:1.45;border:1px solid var(--border);-webkit-overflow-scrolling:touch}
#content code{font-family:var(--font-mono);font-size:13px;background:var(--code);padding:1px 5px;border-radius:3px;color:var(--accent);word-break:break-word}
#content pre code{background:none;padding:0;color:inherit;word-break:normal}
#content table{width:100%;border-collapse:collapse;margin:16px 0;font-size:14px;border:1px solid var(--border);display:block;overflow-x:auto;-webkit-overflow-scrolling:touch}
#content th,#content td{padding:6px 10px;text-align:left;border:1px solid var(--border);white-space:nowrap}
...
#menu-toggle{display:none;position:fixed;top:10px;left:10px;z-index:200;width:40px;height:40px;border:none;border-radius:10px;background:var(--accent);color:#fff;font-size:18px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,.15);-webkit-tap-highlight-color:transparent}
#menu-toggle:active{transform:scale(.92)}
@media(max-width:1023px){
  #sidebar-overlay.show{display:block}
  #sidebar{transform:translateX(-100%);width:300px}
  body.dark #sidebar-overlay{background:rgba(0,0,0,.5)}
  #sidebar.open{transform:translateX(0)}
  #content{margin-left:0;padding:56px 20px 64px}
  #menu-toggle{display:flex;align-items:center;justify-content:center}
  #toc-container a{padding:5px 8px 5px 4px}
  #toc-container .sw a{padding-left:10px}
  .tb{width:22px;height:22px;line-height:22px;font-size:12px}
  #search-results{position:fixed;top:auto;left:0;right:0;max-height:60vh;margin:0 10px}
  #progress{height:3px}
}
@media(max-width:480px){
  #content{padding:52px 14px 56px}
  #content h1{font-size:24px}
  #content h2{font-size:19px;margin:28px 0 8px}
  #content h3{font-size:16px;margin:22px 0 6px}
  #content h4{font-size:15px}
  #content table{font-size:13px}
  #content th,#content td{padding:5px 7px}
  #content blockquote{padding:8px 12px}
  #content pre{padding:10px 12px;font-size:12px}
  #content ul,#content ol{margin-left:18px}
  #menu-toggle{top:8px;left:8px;width:36px;height:36px;font-size:16px}
  #sidebar{width:100%}
}
</style>
</head>
...
<button id="menu-toggle" onclick="t()" aria-label="Открыть навигацию">☰</button>
<div id="sidebar-overlay" onclick="t()"></div>
<div id="sidebar">
  <div id="sidebar-header">
    <div class="logo">SRS</div>
    <button class="theme-btn" onclick="d()" id="theme-switch" aria-label="Переключить тему">●</button>
  </div>
  <div id="search-wrap" style="position:relative">
    <input type="text" id="search" placeholder="Поиск…" aria-label="Поиск">
    <button id="search-clear" onclick="c()" aria-label="Очистить">×</button>
...
<script>
function t(){const s=document.getElementById('sidebar'),o=document.getElementById('sidebar-overlay');s.classList.toggle('open');o.classList.toggle('show')}
...
document.getElementById('toc-container').addEventListener('click',function(e){var a=e.target.closest('a');if(a&&a.getAttribute('href')&&a.getAttribute('href').startsWith('#')){setTimeout(function(){t()},100)}})
