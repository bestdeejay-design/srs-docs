...
#### 1.1.1 О документе: Discovery-фаза
...
- ✅ 9.4 Feature Map
- ✅ 9.5 Developer Q&A (что запросить у разработчика)
...
### 9.4 Feature Map (Карта функциональных требований)
...
### 9.5 Developer Q&A (Что запросить у текущего разработчика)
...
|---|---|
| **Назначение курьера (dispatch)** | Multi-objective optimization: Cheapest Insertion + 2-opt. 6 целей: время доставки, SLA, дистанция, равномерная загрузка, устойчивость к сбоям, зона покрытия. Данные маршрутов в Redis GEO (`GEOADD`/`GEORADIUS`), пересчёт при появлении нового заказа |
...
| **Остатки товаров** | Данные от сетей — через API/парсинг/XLS с периодичностью 1–4 часа. Онлайн-остатки неточны — финальное подтверждение делает пикер в магазине. В БД: `store_prices.quantity` с отметкой `updated_at` |
